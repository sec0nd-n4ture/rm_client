...
class CircularMenu(Container, Interactive):
    def __init__(self, mod_api: ModAPI, parent: Frame):
        ...
        self.buttons: list[CircularMenuButton] = [
            self.button_1, 
            self.top_panel_button, 
            self.button_3, 
            self.button_4, 
            self.button_5
        ]
        image = mod_api.create_interface_image(
            "mod_graphics/rm_logo.png", 
        ...
        self.button_center_offset_x = self.image_width_half - \
            (self.button_1.image.get_dimensions.x / 2) * self.button_1.scale.x

        self.button_center_offset_y = self.image_height_half - \
            (self.button_1.image.get_dimensions.y / 2) * self.button_1.scale.y
    ...
    def debug_area_trigger(self):
        idbg_dot1 = self.mod_api.create_interface_image(
            "mod_graphics/dbg_dot.png", 
            Vector2D(self.area_trigger.corner_top_left.x, self.area_trigger.corner_top_left.y)
        )
        idbg_dot1 = self.mod_api.create_interface_image(
            "mod_graphics/dbg_dot.png", 
            Vector2D(self.area_trigger.corner_top_right.x, self.area_trigger.corner_top_right.y)
        )
        idbg_dot1 = self.mod_api.create_interface_image(
            "mod_graphics/dbg_dot.png", 
            Vector2D(self.area_trigger.corner_bottom_left.x, self.area_trigger.corner_bottom_left.y)
        )
        idbg_dot1 = self.mod_api.create_interface_image(
            "mod_graphics/dbg_dot.png", 
            Vector2D(self.area_trigger.corner_bottom_right.x, self.area_trigger.corner_bottom_right.y)
        )
...
class AccountMenuButton(CircularMenuButton):
    def __init__(self, parent: Frame, mod_api: ModAPI):
        super().__init__(mod_api, parent, "mod_graphics/fa-user-solid.png", "Account")

class TopPanelToggleMenuButton(CircularMenuButton):
    def __init__(self, parent: Frame, mod_api: ModAPI):
        super().__init__(mod_api, parent, "mod_graphics/fa-eye-solid.png", "Hide top panel")
        self.button_icon_toggled = mod_api.create_interface_image(
            "mod_graphics/fa-eye-slash-solid.png", 
            scale=Vector2D(0.35, 0.35)
        )
